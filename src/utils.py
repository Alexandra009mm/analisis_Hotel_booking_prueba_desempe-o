...
def column_normalization(df: pd.DataFrame): #Verify the parameter's data type is dataframe.
    '''
    ...
    EXAMPLE:
        BEFORE: ['Código Cliente', 'Fecha Venta', 'Importe Total Ñoño']
        AFTER: ['codigo_cliente', 'fecha_venta', 'importe_total_nono']
    '''
...
def clean_text_spaces(df: pd.DataFrame):
    ...
    print(
        f'[TRANSFORM] Spaces cleaned in {len(text_columns)} text columns'
    )
